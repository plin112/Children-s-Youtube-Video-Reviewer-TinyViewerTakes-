...
router.get('/register', (req, res) => {
    if (req.session.user) {
        const redirectRoute = req.session.user.role === 'admin' ? '/admin' : '/protected';
...
router.post('/register', async (req, res) => {
  let { firstNameInput, lastNameInput, emailAddressInput, passwordInput, roleInput } = req.body;
    ...
    try {
      ...
      roleInput=validation.checkRole(roleInput);
        ...
        const result = await userData.registerUser(firstNameInput, lastNameInput, emailAddressInput, passwordInput, roleInput);
...
router.post('/login', async (req, res) => {
  let { emailAddressInput, passwordInput } = req.body;
    ...
    try {
        ...
        return res.status(400).render('login', { error: 'Invalid email address or password.', title: 'Login' });
    }
});
